<template>
  <el-form ref="individualizationRef" :rules="individualizationRules" status-icon>
      ...
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="使用网络铃声" name="second">
          <el-input v-model="networkUpload.audioURL" placeholder="请填写网络地址">
            <template #append>
              <el-button @click="handleNetworkUpload">上传</el-button>
            </template>
          </el-input>
        </el-tab-pane>
      </el-tabs>
...
const handleNetworkUpload = async () => {
  if (networkReg.test(networkUpload.audioURL as string)) {
    ...
    if (!res) return ElMessage.error("音频时长不能大于5秒");
